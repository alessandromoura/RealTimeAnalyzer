...
namespace RealTimeAnalyzer.Publisher.ErrorLogging
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var token = cts.Token;
            var errorsTask = Task.Run(async delegate 
            {
                Console.WriteLine("Errors task started!!!");
                while (!cts.IsCancellationRequested)
                {
                    int timeout = new Random().Next(1000, 5000);
        ...
        static ErrorDetail CreateMessage()
        {
            Location loc = RandomizeLocation();

            ErrorDetail ed = new ErrorDetail()
            {
                AppName = RandomizeText("Application", 5),
                Username = RandomizeText("Username", 500),
                Description = RandomizeText("Error type number", 100),
                Region = loc.Region,
                AppType = RandomizeAppType(),
                Date = RandomizeDate(),
